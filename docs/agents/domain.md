...
| `CpalSink` previously owned device format conversion; output planning, terminal conversion, and exact native binding now live in `OutputPlanner`, `FilterChain`, and `OutputSession` respectively. Dynamic Filter registration and async-native Source remain separate known gaps. | `crates/around-engine/src/output_plan.rs`, `crates/around-engine/src/filter_chain.rs`, `crates/around-engine/src/output_session.rs` | Integrate dynamic Filters and implement the async Source contract in follow-up work |

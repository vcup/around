...
The codec hot path uses an ABI‑stable fn‑pointer vtable generated by
`#[stabby::stabby]` rather than a `dyn Codec` trait object. This
eliminates virtual dispatch overhead from the per‑frame `read()` call,
satisfies Constitution §III, and provides compile‑time safety (no
hand‑written `unsafe extern "C"` vtables).

## Status

Superseded by ADR-0004 (stabby‑based extension system).
